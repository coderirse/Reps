...
- **云端题库**：作者上架新题库后，App 内直接下载导入——**不用等发新版**（下载后同样先预览再入库）
...
**你的刷题数据永远不会离开手机**：题库、进度、错题、笔记只存应用私有目录，卸载即清除；App 不设账号、不上传任何内容、无广告无统计。系统云备份保持关闭，换机迁移走应用内 JSON 导出/导入。

App 会联网，但只有两个**只读**出口：**检查应用更新**与**拉取云端题库**。这条边界由构建门禁强制——`./gradlew verifyNetworkContainment` 会检查网络调用只允许出现在 `data/net` 包内，别处出现网络 API 直接让构建失败；同时断言 `INTERNET` 权限确实已声明，防止云功能被静默破坏。
...
- [x] v1.2.0 — 云端题库（作者上架、用户直接下载，无需更新 App）与应用内更新
