...
interface StudySessionDao {
    ...
    /**
     * One active session per subject: entering a new practice marks the old
     * one ABANDONED (not COMPLETED — it was never submitted, and abandoned
     * papers stay out of the history tab).
     */
    @Query("UPDATE study_sessions SET status = 3 WHERE subjectId = :subjectId AND status = 0")
